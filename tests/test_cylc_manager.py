...
def test_parse_profiling_data_missing_files(mock_is_file, mock_path_glob, mock_parse):
    """Test the parse_profiling_data method of PayuManager with missing directories."""

    config_profiling = MockCylcManager(Path("/fake/path"))
...
def test_parse_profiling_data(mock_parse, mock_glob):
    """Test the parse_profiling_data method of PayuManager."""

    config_profiling = MockCylcManager(Path("/fake/path"))
    config_profiling.known_parsers["fake-parser"].read.return_value = {"regions": ["aregion"], tmax: 1}
