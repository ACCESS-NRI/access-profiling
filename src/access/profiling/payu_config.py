...
class PayuConfigProfiling(ConfigProfiling, ABC):
    """Abstract base class to handle profiling data for Payu configurations."""
    ...
    def parse_profiling_data(self, path: Path) -> dict[str, xr.Dataset]:
        ...
        matches = list(archive.glob("payu_jobs/*/run/*.json"))
        ...
        matches = list(archive.glob("output*"))
        if len(matches) == 0:
